...
> **어디에 맞는 도구인가:** 이것은 DALi 프리뷰의 *사람용* 절반 — 빠르게 눈으로 확인하는 루프이지
> 기기 검증을 대체하지 않습니다. 형제 도구 `dali-ui-preview-cli`가 *에이전트용* 절반(PNG + 구조화
> 씬 트리 + verify 루프)입니다. 둘 다 1차 필터이고, 최종 확인은 실기기/에뮬레이터의 몫입니다.
> 전체 근거: [`docs/value-and-positioning.md`](docs/value-and-positioning.md).
