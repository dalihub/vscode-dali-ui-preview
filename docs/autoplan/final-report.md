...
| M3 | 실빌드·렌더 검증 + 매트릭스 + 리포트 | PASS | 1 | (this) | **실제 PNG 렌더 2장**, cross-file 경계 실측 |

전체 loop budget 12 중 4 사용. 동일 테스트 3연속 실패 없음.
...
1. **orchestrator 통합** (compile-probe→Rung3 fallback): `runPreview`에 SliceBuilder를 연결해 사용자가 VS Code에서 실제로 비자기완결 코드를 preview. **자율 사이클에서 보류한 이유**: 실제 live-preview 흐름을 바꾸는 프로덕션 변경이고, external-review가 못박은 "compile-probe 후 실패 시 현행 fallback"(혼란스러운 빌드에러 방지)을 지켜야 하며, VS Code 안에서만 완전 검증 가능 → 회귀 위험이 있어 사용자 확인 후 진행 권장.
2. **Rung1 (clangd cross-file)**: 실전 앱을 풀려면 핵심. 본 검증이 Rung2 경계를 확정했으니 Rung1이 메울 갭이 명확.
3. **시각 정직성 배지**: auto-stub 화면에 "샘플 데이터(자동)" 오버레이.
