...
- **A value or component in a SEPARATE MODULE / LIBRARY** — a different build unit (its header
  reached via a build-system `-I` flag, an SDK / `<system>` include, or any folder **outside
  this project**). This previewer is a **lightweight source slicer, not a full build**: it only
  follows **relative** quoted `#include`s inside this project and cannot cross module/build
  boundaries. (A full-build previewer such as Jetpack Compose *does* resolve cross-module values
  via the build graph; this one does not — so **inline the value**, or move its header where a
  relative `#include` can reach it.)
...
A preview renders **one static frame in a sandbox** — no network, no file/DB access. This is
inherent to static UI preview (**Jetpack Compose Preview has the identical limits**), so don't
try to load real data — approximate it and **tell the human what you approximated**.

- **Runtime / async data** (network, DB, streaming, `GetUser()` …): it can't load. **Inject
  sample data** — build the view with example values (or a sample model) and say "showing
  sample data." (Same in Compose Preview: the render sandbox has no network/file access.)
- **Classes built by dependency injection** (view-models needing repositories / services): the
  previewer can't construct the dependency graph. Pass a **sample instance**, or wrap only the
  view-building part in a small preview factory instead of constructing the whole controller.
  (Same limit as Compose Preview with Hilt view-models.)
- **App singletons / services resolved at runtime**: use literal values. (Theme, locale, and
  size themselves **are** supported — set them with `// @preview-config`; only a value that a
  *runtime* manager fetches falls back.)
- **Live-interaction states — scroll position, selection, hover, text entry**: a static frame
  can't drive these; render the representative state. (Focus and animation are the exception —
  those **are** supported: see `// @preview-state` in the render-configs section below.)
