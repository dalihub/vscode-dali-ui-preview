...
int main(int argc, char* argv[])
{
  ...
  // SetScalingFactor for fontScale) into the chain before Apply(). Empty by
  // default → byte-identical to the M2 harness. (locale=RTL is applied per-view
  // via root LAYOUT_DIRECTION in the POST_BUILD_FOCUS slot, where `root` exists —
  // not here — so a ROW FlexLayout mirrors; WU-M3.5.)
