...
int main(int argc, char* argv[])
{
  ...
  // (ui-config.h), so it must be set BEFORE Apply(). Harmless when no focus
  ...
  // SetScalingFactor for fontScale) on __uiConfig before Apply(). Empty by
  // default. (locale=RTL is applied per-view via root LAYOUT_DIRECTION in the
  // POST_BUILD_FOCUS slot, where `root` exists — not here — so a ROW FlexLayout
  // mirrors; WU-M3.5.) NOTE: dali-ui setters return void (the fluent chaining API
  // was removed), so these are sequential statements on __uiConfig, not a
  // UiConfig::New().SetX()...Apply() chain.
  UiConfig __uiConfig = UiConfig::New();

  __uiConfig.SetAlwaysShowFocus(true);
  __uiConfig.Apply();
